...
func (a *API) getVMParameters(name, userdata, sshkey, storageAccountURI string, ip *armnetwork.PublicIPAddress, nic *armnetwork.Interface) armcompute.VirtualMachine {
	osProfile := armcompute.OSProfile{
		AdminUsername: util.ToPtr("core"),
		ComputerName:  &name,
		LinuxConfiguration: &armcompute.LinuxConfiguration{
			SSH: &armcompute.SSHConfiguration{
				PublicKeys: []*armcompute.SSHPublicKey{
					{
						Path:    util.ToPtr("/home/core/.ssh/authorized_keys"),
						KeyData: &sshkey,
					},
				},
			},
	...
	vm := armcompute.VirtualMachine{
		...
		Properties: &armcompute.VirtualMachineProperties{
			HardwareProfile: &armcompute.HardwareProfile{
				VMSize: util.ToPtr(armcompute.VirtualMachineSizeTypes(a.Opts.Size)),
			},
			StorageProfile: &armcompute.StorageProfile{
				ImageReference: imgRef,
				OSDisk: &armcompute.OSDisk{
					CreateOption: util.ToPtr(armcompute.DiskCreateOptionTypesFromImage),
					DeleteOption: util.ToPtr(armcompute.DiskDeleteOptionTypesDelete),
					ManagedDisk: &armcompute.ManagedDiskParameters{
						StorageAccountType: util.ToPtr(armcompute.StorageAccountTypesPremiumLRS),
					},
				},
			},
			OSProfile: &osProfile,
				...
				NetworkInterfaces: []*armcompute.NetworkInterfaceReference{
					{
						...
						Properties: &armcompute.NetworkInterfaceReferenceProperties{
							Primary:      util.ToPtr(true),
							DeleteOption: util.ToPtr(armcompute.DeleteOptionsDelete),
						},
					},
				},
			},
	...
	case "nvme":
		vm.Properties.StorageProfile.DiskControllerType = util.ToPtr(armcompute.DiskControllerTypesNVMe)
	case "scsi":
		vm.Properties.StorageProfile.DiskControllerType = util.ToPtr(armcompute.DiskControllerTypesSCSI)
	}
...
func (a *API) GetConsoleOutput(name, resourceGroup, storageAccount string) ([]byte, error) {
	...
	vm, err := a.compClient.Get(context.TODO(), vmResourceGroup, name, &armcompute.VirtualMachinesClientGetOptions{
		Expand: util.ToPtr(armcompute.InstanceViewTypesInstanceView),
	})
	if err != nil {
