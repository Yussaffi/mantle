	...
	poller, err := a.depClient.BeginCreateOrUpdate(context.TODO(),
		...
		armresources.Deployment{
			Properties: &armresources.DeploymentProperties{
				Mode:       util.ToPtr(armresources.DeploymentModeIncremental),
				Parameters: params,
				Template:   template,
			},
		},
...
func (a *API) CreateImage(name, resourceGroup, blobURI string) (armcompute.Image, error) {
	...
	poller, err := a.imgClient.BeginCreateOrUpdate(context.TODO(), resourceGroup, name, armcompute.Image{
		...
		Properties: &armcompute.ImageProperties{
			HyperVGeneration: util.ToPtr(armcompute.HyperVGenerationTypes(a.Opts.HyperVGeneration)),
			StorageProfile: &armcompute.ImageStorageProfile{
				OSDisk: &armcompute.ImageOSDisk{
					OSType:  util.ToPtr(armcompute.OperatingSystemTypesLinux),
					OSState: util.ToPtr(armcompute.OperatingSystemStateTypesGeneralized),
					BlobURI: &blobURI,
				},
			},
		},
